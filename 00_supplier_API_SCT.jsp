...
public String imsiCdrQueryForSCT(String imsi, String imsiResponse, String timezone){
	...
	try
	{
		...
		in.close();
		httpRet = buf.toString();	//取得回應值
		writeLog("debug", "SCT server CDR response: " + httpRet);
		
		
		parser	= new JSONParser();
		objResponseBody = parser.parse(httpRet);
		JSONObject jsonObjectResponseBody = (JSONObject) objResponseBody;
		int resResult = ((Long)jsonObjectResponseBody.get("result")).intValue();
		try{
			parser	= new JSONParser();
		...
		writeLog("error", "Exception when send CDR message to SCT server: " + e.toString());
		try{
			parser	= new JSONParser();
			objResponseBody = parser.parse(imsiResponse);
			obj = (JSONObject) objResponseBody;
